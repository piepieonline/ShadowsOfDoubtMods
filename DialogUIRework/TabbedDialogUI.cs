...
// TODO: Long pause when talking to someone for the first time
// TODO: Default to the tab containing the top option
// TODO: Defaulting to all when reentering dialog

public class TabbedDialogUI
{
    private GameObject tabContainer;
    private GameObject contentContainer;
    private Dictionary<string, Il2CppSystem.Collections.Generic.List<DialogButtonController>> categorizedOptions = new Dictionary<string, Il2CppSystem.Collections.Generic.List<DialogButtonController>>();
    ...
    public static void AddIDs(string category, params string[] ids)
    {
        CategoryIDs[category] = ids.ToList();
    }
    
    public static void AddKeywords(string category, params string[] keywords)
    {
        _categoryKeywords[category] = keywords.ToList();
    }
    ...
    private string DetermineCategory(DialogButtonController option)
    {
        ...
        {
            DialogUIReworkPlugin.PluginLogger.LogWarning($"Placed {optionText} by default");
            return "Other";
        }
        ...
        foreach (var kvp in _categoryKeywords)
        {
            ...
            foreach (var keyword in kvp.Value)
            {
                ...
                {
                    DialogUIReworkPlugin.PluginLogger.LogWarning($"Placed {optionText} by keyword ({keyword})");
                    return kvp.Key;
                }
            }
        }
    ...
    private void CreateTab(string category)
    {
        ...
        */
        ...
        tmpText.fontSize = 14; // templateTMPPro.fontSize;
    ...
    {
        if (InteractionController.Instance.talkingTo == null || InteractionController.Instance.talkingTo.speechController.speechActive)
        {
            return;
        }
    ...
    private static Dictionary<string, List<string>> _categoryKeywords = new Dictionary<string, List<string>>
    {
        {
            "Dialogue",
            new List<string>
        ...
        { "Other", new List<string> { "medical", "recover", "escape", "attempt" } },
        { "All", new List<string> { } },
    };
}
